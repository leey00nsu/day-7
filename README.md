...
최초 접속 시 실제 게임에서 사용하는 영상과 오디오 약 36MB를 현재 탭에
미리 다운로드할지 묻습니다. 동의하면 R2와 애플리케이션에서 파일을 받은
뒤 파일별 blob URL로 재생합니다. 준비한 데이터는 탭을 닫거나 새로고침하면
삭제됩니다. 다운로드를 원하지 않거나 실패한 경우에는 기존 네트워크
스트리밍으로 재생합니다. 같은 탭에서는 옵션에서 재생 방식을 다시
선택하거나 준비한 데이터를 삭제할 수 있습니다.
