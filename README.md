...
정규직 전환 결과 발표를 일주일 앞둔 김 인턴.
월요일의 석연치 않은 업무 지시부터 금요일의 최종 면담까지, 매일 조직과
자신 사이에서 선택해야 합니다.

선택은 그 자리에서 끝나지 않습니다. 동료들은 김 인턴의 행동을 기억하고,
...
분기점에서는 김 인턴의 속마음과 두 가지 행동이 함께 나타납니다. 선택
...
- 최초 방문 미디어 다운로드 동의, OPFS 저장 및 스트리밍 폴백
- 파일 크기·SHA-256 검증과 버전별 미디어 캐시 갱신
...
오류 화면을 표시합니다. 배포용으로 최적화한 로컬 복사본은
`public/videos/`, 변환 전 원본은 `video-originals/`에 보관합니다. 두
폴더 모두 `.gitignore`에 등록되어 있으며 애플리케이션 빌드에는 필요하지
않습니다.

최초 접속 시 실제 게임에서 사용하는 영상과 오디오 약 40MB를 기기에
저장할지 묻습니다. 동의하면 R2와 애플리케이션에서 파일을 스트리밍으로
받아 OPFS에 저장하고, 이후에는 파일별 blob URL로 재생합니다. 저장을
원하지 않거나 OPFS를 지원하지 않는 브라우저에서는 기존 네트워크
스트리밍으로 재생합니다. 저장 방식은 옵션에서 다시 선택하거나 내려받은
데이터를 삭제할 수 있습니다.

미디어를 교체했다면 최적화한 `public/videos/`와 `public/audio/`를 기준으로
manifest의 파일 크기와 해시를 다시 생성합니다.

```bash
pnpm media:manifest
```

R2는 서비스 도메인에서 JavaScript `fetch` 요청을 허용하도록 `GET` CORS와
`Content-Length`, `Content-Type`, `ETag` 응답 헤더 노출이 필요합니다.
