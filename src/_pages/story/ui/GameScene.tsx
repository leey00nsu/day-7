...
import {
  StoryMusic,
  type StoryMusicMode,
} from "../model/StoryMusic";
import {
  NarrationAudio,
  type NarrationAudioHandle,
} from "../model/NarrationAudio";
import {
  useHowlerSound,
  useWebAudioSettings,
...
export function GameScene() {
  ...
  const {
    ...
    shouldResume,
    startBuffering,
  } = useStoryMediaBuffering();
  const {
    finishResume,
    pageHidden,
    requireResume,
    resumeRequired,
  } = useStoryPlaybackVisibility({
    isPlaying,
    pausePlayback() {
  ...
  }, [clearBuffering, narrationFilename, sceneKey, videoFilename]);
  ...
  const resumeBufferedMedia = useCallback(async () => {
    if (!isBuffering() || !shouldResume()) return;
